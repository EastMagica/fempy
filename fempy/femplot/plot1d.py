...
def plot_1d(u_lst, mesh, u_true, title):
    ...
    ax0.plot(np.sort(mesh.points), u_lst[np.argsort(mesh.points)], color='r', linestyle='--')
    ax0.scatter(mesh.points, u_lst, color='black')
    # ax0.set_xticks(np.sort(mesh.p_mat))
    ax0.set_xlim(mesh.points[mesh.p_end][0], mesh.points[mesh.p_end][1])
    ...
    ax1.scatter(mesh.points, [0] * mesh.npoints)
    ax1.set_xlim(mesh.points[mesh.p_end][0], mesh.points[mesh.p_end][1])
    fig.suptitle(title)
    plt.show()
