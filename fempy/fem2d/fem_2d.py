...
class FEM2D(object):
    ...
    def assembly_af_1(self):
        """
        Assembly matrix A and F by Gauss p_mat.
        :return:
        """
        for k, v in enumerate(self.mesh.u_mat):
    ...
    def construct_af(self, p):
        ...
        jacobian = area(p[0], p[1], p[2])
        # f mat element (1x3)
        f_lst += np.dot(self.f(q_point[:, 0], q_point[:, 1]), basis_v.T) * self.g_w * jacobian
        # a mat element (3x3)
        for k in range(3):
            for j in range(3):
                a_mat[j, k] += basis_g[j, 0] * basis_g[k, 0] + basis_g[j, 1] * basis_g[k, 1]
        a_mat *= np.sum(self.g_w * jacobian)
        return a_mat, f_lst
    ...
    def error_l2(self):
        ...
        for k, v in enumerate(self.mesh.u_mat):
            ...
            # jacobian = local_to_global_jacobian(p)
            jacobian = area(p[0], p[1], p[2])
            error2 += np.sum((self.eq['u'](q_point[:, 0], q_point[:, 1]) - np.dot(self.u_lst[v], basis_v))**2 * self.g_w * jacobian)
